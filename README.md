...
- **스크롤백 자동 복귀**: 콘솔 스크롤백을 위로 올린 상태에서 타이핑하면 자동으로 하단으로 복귀
...
# 참고: 터미널 크기 변경 후 윈도우 전환 시
# 새 윈도우가 현재 터미널 크기에 맞게 자동 리사이즈됨
```
...
| 기능 | 설명 |
|------|------|
| 화면 분할 | `Ctrl+A, S` (수평), `Ctrl+A, |` (수직) |
| 복사 모드 | `Ctrl+A, [` |
| 멀티 유저 | 여러 사용자가 동시 접속 |
| 로깅 | `Ctrl+A, H` |
| `.screenrc` | 설정 파일 (대신 JSON 프로필 사용) |
...
| 스크롤백 자동 복귀 | 콘솔 스크롤백을 위로 올린 상태에서 타이핑 시 자동 하단 복귀 |
| 자동 서버 시작 | 클라이언트 실행 시 서버 자동 시작 |
| 자동 윈도우 리사이즈 | 윈도우 전환 시 현재 터미널 크기에 맞게 자동 리사이즈 |
